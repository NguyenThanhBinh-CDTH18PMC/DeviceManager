...
namespace DeviceManagerApp.DAO.DataLayerBase
{
    ...
    public class ShipmentDetailDataLayerBase
    {
                ...
                using (SqlCommand command = new SqlCommand(storedProcName, connection))
                {
                    ...
                    using (SqlDataAdapter da = new SqlDataAdapter(command))
                    {
                        ...
                        if (dt != null)
                        {
                            if (dt.Rows.Count > 0)
                            {
                                objShipmentDetailCol = new List<ShipmentDetailModel>();
        ...
        public static List<ShipmentDetailModel> SelectAllDynamicWhere(int? id, string name, string image, string description, DateTime? createdDate, int? createdUserId, bool? isDeleted)
        {
            ...
            if (objShipmentDetailCol != null)
                return objShipmentDetailCol;
            else return new List<ShipmentDetailModel>();
        }
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                ...
                                foreach (DataRow dr in dt.Rows)
                                {
                                    ...
                                    if (dr["Name"] != System.DBNull.Value)
                                        objShipmentDetail.Name = (string)(dr["Name"]);
                                    else
                                        objShipmentDetail.Name = null;
